...
class OfferFormCoachController extends BaseController {
  ...
  _fetchPositions() async {
    try {
      EasyLoading.show();
      var resp = await _positionRequest.getCoachPositions();
  ...
  selectPositions(List<CoachPosition> positions) {
  ...
  offer() async {
    try {
      if (formKey.currentState!.validate()) {
        EasyLoading.show();
        uploading(true);
        await _offerRequest.offerCoach(
          employeeId: coach.value.id,
        ...
        uploading(false);
        Get.back();
        getSnackbar('Informasi', 'Berhasil menawarkan pelatih.');
      }
    } catch (e) {
      uploading(false);
      EasyLoading.dismiss();

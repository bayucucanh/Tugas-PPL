...
import 'package:mobile_pssi/ui/transfer_market/controller/selection_participant.controller.dart';
import 'package:mobile_pssi/utils/ui.spacer.dart';
import 'package:pull_to_refresh/pull_to_refresh.dart';
import 'package:velocity_x/velocity_x.dart';

class SelectionParticipants extends GetView<SelectionParticipantController> {
  static const routeName = '/transfer/market/promo/selection/participants';
  const SelectionParticipants({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    Get.put(SelectionParticipantController());
                    ...
                    return HStack([
                      CircleAvatar(
                        backgroundImage: CachedNetworkImageProvider(
                          participant?.profile?.photo == null
                              ? participant!.profile!.gravatar()
                      ...
                      VStack([
                        ...
                        VStack([
                          (participant.status?.name ?? '-')
                              ...
                              .make(),
                          (participant.profile?.age ?? '-').text.gray500.make(),
                        ]),
                      ]).expand(),
                      UiSpacer.horizontalSpace(space: 10),
